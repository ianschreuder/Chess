class Squares
  ...
  # Get the value of the key given by square
  def self.position(square)
    @@positions[Squares.to_sym(square)]
  end
  ...
  def self.include?(square)
    @@positions.include?(Squares.to_sym(square))
  end
  ...
  # Ensure a square is specified as a symbol
  def self.to_sym(square)
    square = square.downcase.to_sym if square.class == String 
    # square = square.to_s.downcase.to_sym  # necessary?
