...
describe "Bishop" do
  ...
  it "should list all diagonal squares on an empty board" do
    ...
    piece.legal_moves(Position.new([piece]),nil).length.should == 7
    piece.legal_moves(Position.new([piece]),nil).should include(Square.new(:b2))
  ...
  it "should list a square with a opposite-color piece on it as a valid square" do
    ...
    piece1.legal_moves(Position.new([piece1, piece2]),nil).should include(Square.new(:d4))
  end
  
  it "should NOT list a square with a same-color piece on it as valid" do
    piece1 = Bishop.new(:a1, :white)
    piece2 = Pawn.new(:d4, :white)
    piece1.legal_moves(Position.new([piece1, piece2]),nil).should_not include(Square.new(:d4))
  end
