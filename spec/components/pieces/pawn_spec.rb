...
describe "Pawn" do
  ...
  it "should list two forward moves for a non-moved pawn on empty board" do
    ...
    piece.legal_moves(Position.new([piece])).length.should == 2
    piece.legal_moves(Position.new([piece])).should include(Square.new(:b3))
    piece.legal_moves(Position.new([piece])).should include(Square.new(:b4))
  end
  
  it "should list 1 forward move for a moved pawn on an empty board" do
    piece = Pawn.new(:b2, :white)
    piece.move(Square.new(:b3))
    piece.legal_moves(Position.new([piece])).length.should == 1
    piece.legal_moves(Position.new([piece])).should include(Square.new(:b4))
  end
  ...
  it "should not consider off the board a legal move" do
    ...
    piece.legal_moves(Position.new([piece])).length.should == 0
    piece.legal_moves(Position.new([piece])).should_not include(Square.new(:a4))
  end
  ...
  it "should list a square with a opposite-color piece on it as a valid square" do
    ...
    piece1.legal_moves(Position.new([piece1, piece2])).should include(Square.new(:b3))
  end
